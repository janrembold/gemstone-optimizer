...
- **Scintillation (provisional proxy):** `100 (1 − exp(−n/45))`, with `n` the number of active surface facets, **including the 16 planar girdle facets**. Hence our model has n=73 and roughly 80.25, not T57's n=57 and 71.82. This is a geometric density proxy from the requested methodology, not a dynamic light simulation.
...
The public Brilliani sources motivate census sizes, illumination/viewing cones, the tilt ratio, facet-count relationship and weights. The spectral fire algorithm and energy bookkeeping here are independent. Do not interpret equal numeric values as equal physical performance in two different engines.
...
Low-census settings are identical throughout screening within each preset. 8/10/12 diverse finalists are then recomputed with the **same higher census** and full tilt/spectral analysis; only these enter the final Top 5. Advanced final counts must be at least twice the preset screening count; smaller values are rejected. Precision, refinement and region selection remain heuristic: narrow optima can be missed, a candidate may sit at a search boundary, and a tiny score lead can be sampling noise. The UI explicitly avoids implying a proof of global optimality.

The worker yields between candidates, throttles messages to about 150 ms and supports pause/resume. Cancellation terminates the worker immediately. One worker keeps desktop CPU use conservative. Progress/ETA are estimates because local duplicate removal and boundary clipping change work. Pause latency is one candidate; unusually high advanced ray counts extend that latency.
...
3. [Brilliani Labs: methodology overview](https://www.brillianilabs.com/about), [T57 registry](https://www.brillianilabs.com/registry/t57), [BL-M5 registry](https://www.brillianilabs.com/registry/bl-m5). Public protocol and scoring references, not our validation ground truth.
4. [Nassau et al., Synthetic Moissanite, GIA 1997](https://www.gia.edu/gems-gemology/winter-1997-synthetic-moissanite-nassau0) and [issue containing material comparison table](https://www.gia.edu/doc/WN97.pdf). RI/dispersion/anisotropy.
5. [GIA quartz](https://www.gia.edu/rose-quartz), [GIA natural spinel measurements](https://www.gia.edu/doc/WN88.pdf), [GIA topaz](https://www.gia.edu/doc/Pink-Topaz-from-Pakistan.pdf).
6. [SCHOTT optical glass](https://www.schott.com/en-us/products/optical-glass-p1000267), [N-BK7 datasheet](https://media.schott.com/api/public/content/41e799d0bf874807a0bb8e702fbb75b5?v=54856406). Sellmeier coefficients and line indices.
